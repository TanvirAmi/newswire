...
add_theme_support( 'custom-background', apply_filters( 'newswire_custom_background_args', array(
...
function show_page_menu($customClass = 'nav clearfix', $addUlContainer = true, $addHomeLink = true){
	...
	if ($addUlContainer) echo('<ul class="'.$customClass.'">');
		if (get_option($shortname . '_home_link') == 'on' && $addHomeLink) { ?> 
			<li <?php if (is_front_page() || is_home()) echo('class="current_page_item"') ?>><a href="<?php echo esc_url( home_url() ); ?>"><?php _e('Home', 'themejunkie'); ?></a></li>
		<?php };
